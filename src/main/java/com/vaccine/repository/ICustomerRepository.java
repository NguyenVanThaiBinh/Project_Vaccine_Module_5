...
//    Destination Page
    ...
    @Query("SELECT count(c)from  Customer c WHERE c.destination.id=?1 and c.id > ?2 and c.vaccine.id=?3 and c.healthy_status<>3")
    Long countRegister(Long idDes, Long idCus,Long idVac);
